...
//      become placeholders; nested $...$ (text-mode math inside \text{}) stays
//      inside the span verbatim — MathJax's textmacros extension parses it.
...
/**
 * \index{...} inside a math span typesets nothing in print, and MathJax has no
 * \index — left in place it would print as red LaTeX and the entry would never
 * reach the back-of-book index. Lift it out so renderInline collects it from
 * the surrounding text instead.
 */
function hoistIndexEntries(tex) {
  let body = '';
  let hoisted = '';
  let i = 0;
  while (i < tex.length) {
    const at = tex.indexOf('\\index', i);
    if (at === -1) {
      body += tex.slice(i);
      break;
    }
    body += tex.slice(i, at);
    const after = at + '\\index'.length;
    const group = /[a-zA-Z]/.test(tex[after] ?? '')
      ? null // a longer name that merely starts the same way
      : extractBraceGroup(tex, skipWs(tex, after));
    if (!group) {
      body += '\\index';
      i = after;
      continue;
    }
    hoisted += `\\index{${group.value}}`;
    i = group.end;
  }
  return { tex: body, hoisted };
}

/** Register a math span and return the text that replaces it. */
function placeSpan(span, mathSpans) {
  const { tex, hoisted } = hoistIndexEntries(span.tex);
  span.tex = tex;
  mathSpans.push(span);
  return hoisted + PH_OPEN + (mathSpans.length - 1) + PH_CLOSE;
}
  ...
  while (i < text.length) {
    ...
    if (ch === '\\') {
      if (text[i + 1] === '[') {
        ...
        out += placeSpan(
          { kind: 'display', tex: preprocessMathTex(text.slice(i + 2, end)) },
          mathSpans
        );
        i = end + 2;
        continue;
      ...
      if (begin) {
        ...
        if (span) {
          out += placeSpan(span.span, mathSpans);
          i = span.end;
          continue;
        }
      }
    ...
    if (ch === '$') {
      ...
      let j = i + delimLen;
      let found = -1;
      // Depth matters: \text{if $n \ne -1$} nests text-mode math inside the
      // span, and its inner $ must not be read as the closing delimiter.
      let depth = 0;
      while (j < text.length) {
        if (text[j] === '\\') {
          j += 2;
        ...
        else if (depth === 0 && text[j] === '$' && (!isDisplay || text[j + 1] === '$')) {
          found = j;
          break;
        }
        j++;
