...
import { extractBraceGroup, skipWs } from '../parse/tex-utils.js';
...
/** Read one TeX argument: a brace group, or a bare control sequence. */
function readArg(tex, from) {
  const i = skipWs(tex, from);
  const group = extractBraceGroup(tex, i);
  if (group) return { value: group.value, end: group.end };
  // \setlength\fboxsep{1pt} — TeX takes the control sequence as the argument.
  const cs = /^\\(?:[a-zA-Z]+|.)/.exec(tex.slice(i));
  return cs ? { value: cs[0], end: i + cs[0].length } : null;
}

/**
 * Replace every `\name` (with an optional `*`) and its `argc` arguments with
 * `replace(args)`. Regexes cannot do this safely: argument bodies carry braces
 * and commands of their own, and TeX accepts an unbraced control sequence.
 * A malformed occurrence is left alone for the leak check to report.
 */
function rewriteCommand(tex, name, argc, replace) {
  const cmd = `\\${name}`;
  let out = '';
  let i = 0;
  while (i < tex.length) {
    const at = tex.indexOf(cmd, i);
    if (at === -1) {
      out += tex.slice(i);
      break;
    }
    ...
    if (/[a-zA-Z]/.test(tex[j] ?? '')) {
      // a longer name that merely starts the same way (\vspacefoo)
      out += cmd;
      i = j;
      continue;
    }
    if (tex[j] === '*') j++;
    const args = [];
    for (let k = 0; k < argc && j !== -1; k++) {
      const arg = readArg(tex, j);
      if (!arg) {
        j = -1;
        break;
      }
...
const dropCommand = (tex, name, argc) => rewriteCommand(tex, name, argc, () => '');

/**
 * pifont \ding{n} → the Zapf Dingbats glyph. Unicode lays the font out at
 * U+2700 (printable ASCII range) and U+2761 (the upper range).
 */
function dingbat(code) {
  const n = Number(code.trim());
  if (n >= 33 && n <= 126) return `\\unicode{x${(0x2700 + n - 32).toString(16)}}`;
  if (n >= 161 && n <= 254) return `\\unicode{x${(0x2761 + n - 161).toString(16)}}`;
  return '';
}

/** Preprocess math TeX for MathJax (applied to span.tex at protect time). */
export function preprocessMathTex(tex) {
  // Print-only layout inside math: page geometry MathJax has no notion of, and
  // which it would otherwise render as red LaTeX.
  let out = dropCommand(tex, 'setlength', 2);
  out = dropCommand(out, 'vspace', 1);
  out = dropCommand(out, 'hspace', 1);
  out = rewriteCommand(out, 'ding', 1, ([code]) => dingbat(code));
  // fancybox \ovalbox around a calculator key. \enclose needs math mode and
  // these boxes usually sit inside a \text{...} body, so re-enter math with
  // the $ the source already carries around the key's label.
  out = rewriteCommand(out, 'ovalbox', 1, ([body]) => {
    const inner = body.match(/\$([^$]*)\$/);
    const label = inner ? inner[1] : body;
    return `$\\enclose{roundedbox}{${label}}$`;
  });
  return out
    .replace(/\\hfill\b\s*/g, '')
    ...
    // \qed / \qedhere inside math → textual qed mark (matches the print's
    // sans-serif "qed"); \qedhere only differs in where amsthm puts it.
    // \textsf, not \text: the marker often sits inside a \text{...} body,
    // where a nested \text is a MathJax error but \textsf is valid in both
    // modes.
    .replace(/\\qedhere\b/g, '\\textsf{qed}')
    .replace(/\\qed\b/g, '\\textsf{qed}')
 ...
 * Nested $...$ survives verbatim: the [tex]/textmacros extension parses it.
 */
...
export function mathSpanHtml(span) {
  ...
  // env: numbered \tag injection + finalizeMathTex. MathJax's
  // processEnvironments finds \begin{align} etc. on its own, but displaymath —
  // LaTeX's \[ ... \] spelled long, unwrapped by finalizeMathTex — needs the
  // delimiters back: its body may set two environments side by side
  // (\begin{bmatrix}…\end{bmatrix} \quad\text{or}\quad \begin{pmatrix}…),
  // and processEnvironments would leave everything after the first as prose.
  if (span.env === 'displaymath' || span.env === 'displaymath*') {
