...
\begin{exmp}
 ...
 \begin{enumerate}[(a)]
  ...
  \item The distance between the points $P = (2, -1, 4)$ and $Q = (4, 2, -3)$ in $\Real{3}$.\\\emph{Solution:}
...
\begin{figure}[h]
 ...
 \end{center}
 \caption[]{\quad Proof of Theorem \ref{thm:dist3}: $d = \norm{\textbf{w} - \textbf{v}}$}
 \label{fig:dist3p}
...
\resume{enumerate}[{[\bfseries 1.]}]
 ...
 \item Let $\textbf{r}(t)$ be the position vector for a particle moving in $\Real{3}$. Show that
  ...
  \]
 \item Let $\textbf{r}(t)$ be the position vector in $\Real{3}$ for a particle moving at constant speed $c > 0$
  ...
  \perp \textbf{v}(t)$ \emph{and} $\textbf{a}(t) \perp \textbf{v}(t)$).
%  , \emph{and hence} $\textbf{a}(t) \parallel \textbf{r}(t)$.)
